...
public final class SileroVADModel {
    ...
    public static func fromPretrained(
        ...
        switch engine {
        case .mlx:
            progressHandler?(0.5, "Loading MLX model...")
            
            let network = SileroVADNetwork()
            try SileroWeightLoader.loadWeights(model: network, from: modelDir)
        ...
        case .coreml:
            #if canImport(CoreML)
            progressHandler?(0.5, "Loading CoreML model...")
            
            let modelURL = modelDir.appendingPathComponent("silero_vad.mlmodelc", isDirectory: true)
            guard FileManager.default.fileExists(atPath: modelURL.path) else {
                throw AudioModelError.modelLoadFailed(
                    modelId: "Silero-VAD",
                    reason: "CoreML model not found at \(modelURL.path)")
